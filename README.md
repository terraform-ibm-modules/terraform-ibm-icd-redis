...
|------|-------------|------|---------|:--------:|
| <a name="input_access_tags"></a> [access\_tags](#input\_access\_tags) | Add access management tags to the Redis instance to control access. [Learn more](https://cloud.ibm.com/docs/account?topic=account-tag&interface=ui#create-access-console). | `list(string)` | `[]` | no |
...
| <a name="input_backup_crn"></a> [backup\_crn](#input\_backup\_crn) | The CRN of a backup resource to restore from. The backup is created by a database deployment with the same service ID. The backup is loaded after provisioning and the new deployment starts up that uses that data. For classic instances, a backup CRN is in the format crn:v1:<…>:backup:. For Gen2 instances, both coupled (crn:v1:<…>:backup:) and decoupled (crn:v1:bluemix:public:databases-independent-backups:<region>:...) backup CRNs are supported. Classic backup CRNs cannot be used to restore Gen2 instances. If omitted, the database is provisioned empty. | `string` | `null` | no |
...
| <a name="input_resource_tags"></a> [resource\_tags](#input\_resource\_tags) | Add user resource tags to the Redis instance to organize, track, and manage costs. [Learn more](https://cloud.ibm.com/docs/account?topic=account-tag&interface=ui#tag-types). | `list(string)` | `[]` | no |
