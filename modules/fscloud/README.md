...
The IBM Cloud Framework for Financial Services mandates the application of an inbound network-based allowlist in front of the IBM Cloud Databases for (ICD) Redis instance. You can comply with this requirement by using the `cbr_rules` variable in the module, which can be used to create a narrow context-based restriction rule that is scoped to the Redis instance. CRA does not currently support checking for context-based restrictions, so you can ignore the failing goal after you set the context-based restriction.
