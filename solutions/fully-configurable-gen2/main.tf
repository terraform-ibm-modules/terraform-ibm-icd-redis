...
resource "ibm_iam_authorization_policy" "kms_policy" {
  ...
  description            = "Allow all Redis instances in the account ${local.account_id} to read the ${local.kms_service} key ${local.kms_key_id} from the instance GUID ${local.kms_instance_guid}"
  resource_attributes {
    name     = "serviceName"
