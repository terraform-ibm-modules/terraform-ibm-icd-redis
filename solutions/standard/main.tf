...
locals {
  ...
  create_cross_account_kms_auth_policy        = var.existing_db_instance_crn == null && !var.skip_redis_kms_auth_policy && var.ibmcloud_kms_api_key != null && !var.use_ibm_owned_encryption_key
  create_cross_account_backup_kms_auth_policy = var.existing_db_instance_crn == null && !var.skip_redis_kms_auth_policy && var.ibmcloud_kms_api_key != null && !var.use_ibm_owned_encryption_key && var.existing_backup_kms_key_crn != null
  ...
  kms_key_crn       = var.existing_db_instance_crn != null || var.use_ibm_owned_encryption_key ? null : var.existing_kms_key_crn != null ? var.existing_kms_key_crn : module.kms[0].keys[format("%s.%s", local.redis_key_ring_name, local.redis_key_name)].crn
  kms_key_id        = var.existing_db_instance_crn != null || var.use_ibm_owned_encryption_key ? null : var.existing_kms_key_crn != null ? module.kms_key_crn_parser[0].resource : module.kms[0].keys[format("%s.%s", local.redis_key_ring_name, local.redis_key_name)].key_id
...
locals {
  ...
  # tflint-ignore: terraform_unused_declarations
  validate_existing_instance_region = var.existing_db_instance_crn != null && var.region != local.existing_redis_region ? tobool("The region detected in the 'existing_db_instance_crn' value must match the value of the 'region' input variable when passing an existing instance.") : true
}
...
locals {
  redis_guid     = var.existing_db_instance_crn != null ? data.ibm_database.existing_db_instance[0].guid : module.redis[0].guid
